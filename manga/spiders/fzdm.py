...
class FzdmSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield SplashRequest(base, self.parseBook, args={'wait': 30})
        
    def parseBook(self, response):
        books = response.css('#content li a::text').getall()
        urls = response.css('#content li a::attr("href")').getall()
        for i, url in enumerate(urls):
            request = SplashRequest(response.url + str(url), self.parsePage, args={'wait': 30})
    ...
    def parsePage(self, response):
        ...
        if '下一页' == response.css('.navigation .pure-button.pure-button-primary::text').getall()[-1]:
            ...
            request = SplashRequest(response.meta['url'] + next_url, self.parsePage, args={'wait': 30})
